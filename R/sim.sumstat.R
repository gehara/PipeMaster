...
sim.ms.sumstat<-function(model,use.alpha=F,nsim.blocks=1,sim.block.size=1000,path=getwd(),perpop.SS=T,overall.SS=T,
  ...
  .Deprecated("sim.sumstat", package = "PipeMaster",
              msg = "sim.ms.sumstat() is deprecated. Use sim.sumstat() or sim.msABC.sanger() instead, which use the built-in msABC C code and do not require PopGenome.")
  if (!requireNamespace("PopGenome", quietly = TRUE)) {
    stop("sim.ms.sumstat() requires the PopGenome package. Please use sim.sumstat() instead.")
  }
