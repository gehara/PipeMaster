...
#' @param min_per_pop Minimum sample size required per population (default 2).
#'   Populations need at least 2 samples for within-pop statistics (pi, Tajima's
#'   D, Fst) to be defined. Set to a smaller value only if you know what you are
#'   doing; setting to 0 allows the optimizer to drop populations entirely.
...
#' @export
optimize.sfs.model <- function(model, target = NULL, min_per_pop = 2L) {

  if(class(model) != "Model") {
    stop("Argument should be an object of class Model generated by the main.menu.gui() function.")
  }
  if(!is.numeric(min_per_pop) || length(min_per_pop) != 1L || min_per_pop < 0)
    stop("min_per_pop must be a single non-negative number.")
  min_per_pop <- as.integer(min_per_pop)
  ...
  # Diagnose populations that can never meet min_per_pop
  pop_max <- apply(pop_sizes, 2, max)
  bad_pops <- which(pop_max < min_per_pop)
  if(length(bad_pops) > 0) {
    stop(sprintf(
      "Population(s) %s have max %d samples across loci, below min_per_pop=%d.\n  Drop these populations from the model or lower min_per_pop.",
      paste(bad_pops, collapse = ", "),
      max(pop_max[bad_pops]),
      min_per_pop), call. = FALSE)
  }
  ...
  if(is.null(target)) {
    ...
    # Restrict candidate targets to values >= min_per_pop so the optimizer
    # cannot pick a degenerate solution that drops a population entirely.
    unique_per_pop <- lapply(1:npop, function(p) {
      vals <- sort(unique(pop_sizes[, p]))
      vals <- vals[vals >= min_per_pop]
      vals
    })
    ...
    bad <- which(target < min_per_pop)
    if(length(bad) > 0) {
      stop(sprintf(
        "target for population(s) %s is below min_per_pop=%d. Raise the target or lower min_per_pop.",
        paste(bad, collapse = ", "), min_per_pop), call. = FALSE)
    }
