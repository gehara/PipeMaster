...
void sfs_accumulate(char **list, int nsam, int segsites, int *config, int npop) {
    ...
    if (sfs_mode_active == 1) {
        /* ---- 1D SFS (single population), folded ---- */
        /* Fold: use minor allele frequency to match obs_sfs_call().
         * SFS has nsam-1 bins; after folding, bins > nsam/2 are always 0. */
...
/*
 * msABC_combined_batch_call - .Call() entry point for combined sumstat + SFS simulation
 *
 * Produces BOTH text summary statistics AND SFS from a single coalescent pass,
 * by setting sfs_combined_mode = 1 which prevents the continue statements in ms.c
 * that normally skip text output when SFS mode is active.
 *
 * Arguments:
 *   commands_sexp:    character vector of length nsims
 *   mu_rates_sexp:    numeric matrix (locfile_rows x nsims)
 *   rec_rates_sexp:   numeric matrix (locfile_rows x nsims), or NULL
 *   pop_sizes_sexp:   integer vector of per-population sample sizes
 *   one_snp_sexp:     logical, TRUE to sample one SNP per locus
 *   method_sexp:      "stochastic" or "expected"
 *   monomorphic_sexp: logical (unused in C, handled in R)
 *
 * Returns: 2-element R list: list(sumstats_text, sfs_matrix)
 *   sumstats_text: character vector of length nsims
 *   sfs_matrix:    numeric matrix, nsims rows x accum_len columns
 */

/* rec override globals in ms.c */
...
SEXP msABC_combined_batch_call(SEXP commands_sexp, SEXP mu_rates_sexp,
    ...
    /* Validate inputs */
    if (!isString(commands_sexp) || length(commands_sexp) < 1) {
        Rf_error("msABC_combined_batch_call: 'commands' must be a character vector");
    }
    if (!isReal(mu_rates_sexp) || !isMatrix(mu_rates_sexp)) {
        Rf_error("msABC_combined_batch_call: 'mu_rates' must be a numeric matrix");
    }
    if (!isInteger(pop_sizes_sexp) || length(pop_sizes_sexp) < 1) {
        Rf_error("msABC_combined_batch_call: 'pop_sizes' must be an integer vector");
    }
    if (!isLogical(one_snp_sexp) || length(one_snp_sexp) != 1) {
        Rf_error("msABC_combined_batch_call: 'one_snp' must be a single logical value");
    }
    ...
    if (mu_ncol != nsims) {
        Rf_error("msABC_combined_batch_call: mu_rates ncol (%d) != length(commands) (%d)",
                 mu_ncol, nsims);
    }
    ...
    if (has_rec) {
        if (!isReal(rec_rates_sexp) || !isMatrix(rec_rates_sexp)) {
            Rf_error("msABC_combined_batch_call: 'rec_rates' must be a numeric matrix or NULL");
        }
        SEXP rec_dim = getAttrib(rec_rates_sexp, R_DimSymbol);
        rec_nrow = INTEGER(rec_dim)[0];
        int rec_ncol = INTEGER(rec_dim)[1];
        if (rec_ncol != nsims) {
            Rf_error("msABC_combined_batch_call: rec_rates ncol (%d) != nsims (%d)",
                     rec_ncol, nsims);
        }
    ...
    double *accum = (double *)calloc(accum_len, sizeof(double));
    if (accum == NULL) {
        Rf_error("msABC_combined_batch_call: failed to allocate SFS accumulator");
    }

    /* Allocate R results: text vector + SFS matrix + 2-element list */
    SEXP text_result = PROTECT(allocVector(STRSXP, nsims));
    SEXP sfs_result = PROTECT(allocMatrix(REALSXP, nsims, accum_len));
    double *sfs_result_data = REAL(sfs_result);
    SEXP result_list = PROTECT(allocVector(VECSXP, 2));

    /* Set up global SFS mode + combined mode */
    sfs_mode_active = (npop == 1) ? 1 : 2;
    sfs_one_snp = one_snp;
    sfs_npop = npop;
    sfs_pop_sizes = pop_sizes;
    ...
    for (int sim = 0; sim < nsims; sim++) {
        ...
        /* Set mu override for this simulation */
        frag_mu_override = mu_data + (long)sim * mu_nrow;
        frag_mu_override_len = mu_nrow;

        /* Set rec override if provided */
        if (has_rec) {
        ...
        if (argc == 0) {
            ...
            sfs_pop_sizes = NULL;
            UNPROTECT(3);
            Rf_error("msABC_combined_batch_call: failed to parse command for sim %d", sim + 1);
        }

        /* Reset global state and init output capture */
        msABC_reset_streec_statics();
        msABC_init_output_stream();
        ...
        if (jmpval != 0) {
            ...
            sfs_pop_sizes = NULL;
            UNPROTECT(3);
            Rf_error("msABC_combined_batch_call: simulation error at sim %d (exit code %d)",
                     sim + 1, jmpval);
            return R_NilValue;
        }

        /* Run simulation (produces both text output and SFS) */
        msABC_main(argc, argv);
        ...
        if (output != NULL && outlen > 0) {
            SET_STRING_ELT(text_result, sim, mkCharLen(output, (int)outlen));
        } else {
            SET_STRING_ELT(text_result, sim, mkChar(""));
        }

        msABC_close_output_stream();
        free_argv(argc, argv);

        /* Copy SFS to result matrix (column-major) */
        for (int j = 0; j < accum_len; j++) {
            sfs_result_data[j * nsims + sim] = accum[j];
        }
    ...
    /* Build result list */
    SET_VECTOR_ELT(result_list, 0, text_result);
    SET_VECTOR_ELT(result_list, 1, sfs_result);
    ...
    for (int loc = 0; loc < nloci; loc++) {
        ...
        if (npop == 1) {
            for (int si = start_snp; si < end_snp; si++) {
                ...
                /* Fold to minor allele frequency */
                freq = (freq <= nsam - freq) ? freq : nsam - freq;
                if (freq >= 0 && freq <= nsam) {
                    accum[freq] += 1.0;
                }
            }
...
SEXP obs_sfs_vcf_call(SEXP vcf_path, SEXP sample_pop_map, SEXP sample_col_indices,
    ...
    while (1) {
        ...
        /* Polarize by major allele: fold to minor allele side */
        if (total_alt > total_ref) {
            for (int p = 0; p < npop; p++) {
                pop_alt_counts[p] = pop_sizes[p] - pop_alt_counts[p];
            }
        } else if (total_alt == total_ref && npop > 1) {
            /* Lexicographic tiebreak for multi-pop */
            int do_flip = 0;
            for (int p = 0; p < npop; p++) {
                int comp = pop_sizes[p] - pop_alt_counts[p];
                if (pop_alt_counts[p] > comp) { do_flip = 1; break; }
                if (pop_alt_counts[p] < comp) { break; }
            }
            if (do_flip) {
                for (int p = 0; p < npop; p++)
                    pop_alt_counts[p] = pop_sizes[p] - pop_alt_counts[p];
