...
#define EXIT_MSABC(code) longjmp(msABC_jmpbuf, (code) ? (code) : 1)
#else
#define EXIT_MSABC(code) exit(code)
#endif

#define ERROR(message) fprintf(stderr,message),NL,EXIT_MSABC(1)
...
#ifdef R_PACKAGE_BUILD
void msABC_reset_streec_statics(void) {
    if (nnodes != NULL) { free(nnodes); nnodes = NULL; }
    if (chrom != NULL) { free(chrom); chrom = NULL; }
    if (seglst != NULL) { free(seglst); seglst = NULL; }
    seglimit = SEGINC;
    maxchr = 0;
}
#endif
